...
export class HypothesisService {
  ...
  searchAnnotations(data?: { user?: string }): Observable<Annotation[]> {
    ...
    return this.http.get<Annotation[]>(environment.hypothesis.searchAnnotations, { params })
      .pipe(
        map(data => data['rows'])
      );
  }

  updatedAnnotation(id: string, data: { tags: string[] }): Observable<Annotation> {
    return this.http.patch<Annotation>(environment.hypothesis.updateAnnotation.replace(':id', id), data);
  }
}
